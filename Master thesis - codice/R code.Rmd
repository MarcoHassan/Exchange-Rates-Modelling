...
Helper Functions for the global evironment
```{r}
## Function 1 - helps to evalue paste0 expressions and to evalue them directly without 
...
clean <- function() {
  ENV <- globalenv()
  ll <- ls(envir = ENV)
  ll <- ll[!(ll %in% c("data", "clean", "import", "write", "evalue"))]
...
## Remove acf_plots from clean function
clean <- function() {
  ENV <- globalenv()
  ll <- ls(envir = ENV)
            ...
            function(LAG)
            {
              ...
              for( country in c("CH", "JP", "UK") )
              {
                for( method in c("rw", "str", "var") )
                {
                  ...
                  error_data <- tail(data[, paste0("EX_", country, "US")],
...
Perform Directional Accuracy test
```{r}
errorDacFun <- 
            function(LAG)
            {
...
===================================================================
Perform a Diebold Mariano Test to check at equal predictivity power
===================================================================
