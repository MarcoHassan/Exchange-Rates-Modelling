...
write <- function(dat)
  ...
  ## as.dat.frame is necessary to get the date index out of the xts
  }

import <- function(dat)
  {
  dat <- read.csv("exchange rates.csv", header = T) ## loose xts notation.
  dat <- as.xts(dat[,-1], order.by = as.Date(dat[,1]))  
  }
```
