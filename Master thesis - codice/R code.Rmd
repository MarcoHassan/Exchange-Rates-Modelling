...
trend_UN <- ma(na.trim(data$UN_US), order = 12, centre = T)

Seasonality <- na.trim(data$UN_US) - trend_UN

Seasonality <- na.contiguous(Seasonality)

plot(Seasonality) ## additive model; make it integrated and graph of four unemployment series.

## check for autocorrelation in the detrended times series

acf(Seasonality, type = "correlation") ## strong evidence for seasonality. 
