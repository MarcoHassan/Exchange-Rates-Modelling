...
```{r}
data <- na.trim(data)
rm(list=setdiff(ls(), c("data", "acf_plots")))
```
...
Not good better to differenciate annually. do it tomorrow. If you want to be fancy differenciate according to FFT frequency. Notice calling the decompose function it just demeans the series by averaging the period observations. Test with nsdiffs if there is still seasonality or work with heuristics.
```{r}
seasonality_12 <- function(x){
               dat <- ts(x, frequency = 12) ## frequency specifies seasonality
               ...
               if(res == 1) {
                 #data[-1:-12,colnames(x)] <- diff(x, lag = 12)
...
seasonality <- function(x){
               ...
               dat <- ts(x, frequency = time[1]) ## this not good. seasonality not 
...
### not good frequency is specific and not given in months
...
prova <- function(x){
  ...
  bp_ts <- breakpoints(as.ts(data$EX_UKUS, frequency = 12) ~ 1)
...
This section starts with the real analysis of the cleaned dataset.
