...
```{r}
## notice that you should still adjust the exogenous data frame. does not include the trade in the USA which should be included under the hooper merton model


## estimate optimal ARIMAX for UK 
...
for(i in seq(along=nd_aic)) nd_aic[i]<-AIC(arimax(data[1:three_fourth, "EX_UKUS"],unlist(nd_pars[i,1:3]), xreg = data[1:three_fourth, interestUK], method = 'ML'))
nd_pars[which.min(nd_aic),]


## specify the best fit model
order <- nd_pars[which.min(nd_aic),]
model <- arimax(data[1:three_fourth, "EX_UKUS"], order = unlist(order), xreg = data[1:three_fourth, interestUK], method = 'ML')

predict(model, n.ahead = 12, newxreg = data[(three_fourth+1):(three_fourth+12), interestUK])$pred[c(1,3,6,12)] ## subset at the end to get the frequencies of interest, i.e. 1, 3, 6, 12
```
...
exogenous <- !str_detect(colnames(data), "^rw|^EX")
