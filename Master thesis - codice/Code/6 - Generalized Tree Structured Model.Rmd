...
newdata <- cbind(1, exogen[147,-ncol(exogen)])
...
for(country in c("CH", "UK", "JP"))
{
  assign(paste0("interest", country), 
         ...
         !str_detect(colnames(data2), "^rw|^str|^var|^vecm|^GTS|^M1")) ## With M3
}
```
...
## devi rimuovere tutti i max_lik dall global
...
```{r}
fileout <- file("GTS_out.txt", open = "wt")

sink(file = fileout, append = T, type = "message")
  ...
  {  
    ...
    for( iRow in 1:(nrow(data2)-st) )
    {
      ...
      if(is.null(max_lik2))
      {
        ...
        {fcast1 <- xnew %*% max_lik1$right_model} else{
          fcast1 <- xnew %*%  max_lik1$left_model
        }
...
Write Data to csv
```{r}
write(data2, "level_GTS_pred.csv")
```
