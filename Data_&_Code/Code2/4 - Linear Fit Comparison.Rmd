...
# To work with xts times series objects
library(xts)
...
lines(data$OLSPred1_CHUS, col = "salmon", lwd = 2)
...
RMSE <- matrix(NA,4,3, 
               dimnames = list( c("rw", "str", "var", "OLS"),
               c("CH", "JP", "UK") )
               )
...
errorFun <- 
            function(LAG)
            {
              ...
              {
                for( method in c("rw", "str", "var", "OLS") )
                {
                  iRow = iRow + 1
...
u <- na.trim(cbind(data$rwPred1_CHUS, data$varPred1_CHUS, data$strPred1_CHUS, data$OLSPred1_CHUS))
...
for (j in 1:4)
{
  perf[,j]<- abs(tail(data$EX_CHUS, n)-u[,j]) #MAE
...
Iterate without transfer function
```{r}
#Create u a matrix with all different univariate GARCH predictions: each column, one model prediction
u <- na.trim(cbind(data$rwPred1_CHUS, data$varPred1_CHUS, data$OLSPred1_CHUS))
