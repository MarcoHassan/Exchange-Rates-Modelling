...
In contrast to Meese and Rogoff I decided to make use of the ARIMAX transfer function here. This will allow the introduction of lagged exogenous variables and its implementation is favoured compared to the long lagged AR model used by Meese and Rogoff to my mind.
...
for(country in c("CH", "UK", "JP"))
{
  assign(paste0("interest", country), 
         !str_detect(colnames(data2), paste0("^M1|^EX|^EX+\\D", country)) &
...
## Notice that Arimax without an order specification corresponds to a simple OLS model. Implement this.

## Arimax with order specification corresponds to a transfer function model.

____________
(Literature transfer function Model)
____________
...
```{r}
file_con <- file("level_arimax.txt")
sink(file_con, "out")
...
Write the data
```{r}
write(data2, "after_str_level_linear_pred.csv")
```
...
```{r}
## Insert your path here
path <- "/Users/Marco Hassan/Exchange-Rates-Modelling/Master thesis - codice/Written\ Data/after_str_level_linear_pred.csv"
data2 <- read.csv(path)
data2 <- as.xts(data2[,-1], order.by = as.Date(data2[,1]))
```
...
for(country in c("CH", "UK", "JP"))
{
  assign(paste0("interest", country), 
         !str_detect(colnames(data2), paste0("^M1|^EX|^EX+\\D", country)) &
          str_detect(colnames(data2), paste0("\\D", country)) & 
         !str_detect(colnames(data2), "^rw|^str|^OLS"))
}
```
...
exogenous <- !str_detect(colnames(data2), "^M1|^rw|^EX|^str|^OLS|^var")
