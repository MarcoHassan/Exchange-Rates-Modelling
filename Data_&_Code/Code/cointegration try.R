...
fork <- 
  function(dat, split_model, total_residual = 0, grid_level = 16, node_residual = 0, mmPartition)
  {
    improvement <- FALSE
    
    if(mmPartition == 1)
      {
        Log_likelihood <- logLikelihood(as.data.frame(split_model$residuals[,ncol(split_model$residuals)]))
    } else{
    ...
    for (iVariable in c(1:nrow(partitioned_space)))
    {
      ...
      for(jBreak in c(1:ncol(partitioned_space)))
      {
        ...
        #if( nrow(part_right) < 80 ||  nrow(part_left) < 80 ) 
        #{
        #  warning(
        #    sprintf(
        ...
        ## If partition contains less than 80 observations don't even try to estimate the model.
        if( nrow(part_right) < 70 ||  nrow(part_left) < 70 ) 
        {
          next
        }
        ...
        right_res <- lineVar(part_right[, -c(ncol(dat)-1)], 1,lag = 1,
                             include = "none", model = "VAR",
                             estim = "ML", LRinclude = "none")$residuals[,ncol(split_model$residuals)]
        
        left_res <- lineVar(part_left[,-c(ncol(dat)-1)], 1,lag = 1,
                            include = "none", model = "VAR",
                            estim = "ML", LRinclude = "none")$residuals[,ncol(split_model$residuals)]
        ...
        if(part_logLik > Log_likelihood)
        {
          ...
          right_model <- lineVar(part_right[, -c(ncol(dat)-1)], 1,lag = 1,
                                 include = "none", model = "VAR",
                                 estim = "ML", LRinclude = "none")
          
          left_model  <- lineVar(part_left[,-c(ncol(dat)-1)], 1,lag = 1,
                                 include = "none", model = "VAR",
                                 estim = "ML", LRinclude = "none")
...
GTS <- function(dat, max_Partition = 8)
{
  ...
  global <<- lineVar(dat[,-c(ncol(dat)-1)], 1,lag = 1,
                     include = "none", model = "VAR",
                     estim = "ML", LRinclude = "none")
  
  mPartition = 1
  max_lik1 <<- fork(dat, global, mmPartition = mPartition)
  ...
  for(mPartition in 2:max_Partition)
  {
    ...
    assign(paste0("max_lik", mPartition+1 ,"right"), fork(get(paste0("part_right", mPartition))[[1]], model, 
                                                          model$total_residuals, node_residual = get(paste0("part_right", mPartition))[[2]]), mmPartition = mPartition)
    
    assign(paste0("max_lik", mPartition +1,"left"),fork(get(paste0("part_left", mPartition))[[1]], model, 
                                                        model$total_residuals, node_residual = get(paste0("part_left", mPartition))[[2]]), mmPartition = mPartition)
    ...
    for(iNode in mylist)
    {
      new_list[[i]] <- fork(iNode$input_data, model, model$total_residuals, 
                            node_residual = iNode$node_residuals, mmPartition = mPartition)
      i=i+1
    }
...
prunedTree <- function()
{
  ...
  global$loglik <- logLikelihood(as.data.frame(global$residuals[,ncol(global$residuals)]))
...
GTSEstimate <- function(country, lag_term)
{
  if(is.null(condition))
  {
    model <- lineVar(xshort[,-c(ncol(xshort))], 1,lag = 1,
                     include = "none", model = "VAR",
                     estim = "ML", LRinclude = "none")
    
    prediction<- predict_rolling(model, nroll = lag_term, n.ahead = lag_term)
    return(prediction$pred[LAG, ncol(xshort)-1])
  }
  
  ## Check in which branch the observation falls [Condition 1]
  if(xshort[LAG, (prova$nodes[1][[1]]$split$varid+1)] >
  ...
  if(!is.null(prova$nodes[next_node_id][[1]]$info))
  {
    ...
    ## at the end node
    prediction<- predict_rolling(prova$model[next_node_id][[1]], nroll = lag_term, n.ahead = lag_term)
    return(prediction$pred[LAG, ncol(xshort)-1])
  } 
  ...
  while(loop)
  {
    if(xshort[LAG, (prova$nodes[next_node_id][[1]]$split$varid+1)] > 
