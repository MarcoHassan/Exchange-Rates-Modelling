...
===========
```{r}
path <- "/Users/Marco Hassan/Exchange-Rates-Modelling/Data_&_Code/Written\ Data/stationary_GTSVAR_pred.csv"
...
for(jCountry in c("CH", "JP", "UK"))
{
  ...
  print(lines(data[,paste0("GTS_VARPred1_", jCountry, "US")], col = "violet", lwd = 2))
...
errorFun <- 
            function(LAG)
            {
              ...
              {
                for( method in c("rw", "str", "OLS", "GTS_VAR") )
                {
                  iRow = iRow + 1
...
u <- na.trim(cbind(data$rwPred1_UKUS, data$OLSPred1_UKUS, data$GTS_VARPred1_UKUS))
...
for(country in c("CH", "JP", "UK"))
{
  ...
  for( LAG in c("1", "3", "6", "12") )
  {
        ...
        prova <- read_file(paste0("Split/VAR/GTSVAR_", country, "_", LAG, ".txt"))
