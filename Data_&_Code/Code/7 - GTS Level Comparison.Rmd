...
===========
```{r}
path <- "/Users/Marco Hassan/Exchange-Rates-Modelling/Master thesis - codice/Written\ Data/stationary_GTS_pred.csv"
...
RMSE <- matrix(NA,4,3, 
               dimnames = list( c("rw", "str", "var", "GTS"),
               c("CH", "JP", "UK") )
               )
...
errorFun <- 
            function(LAG)
            {
              ...
              {
                for( method in c("rw", "str", "var", "GTS") )
                {
                  iRow = iRow + 1
...
```{r}
for( iLAG in c(1,3,6) )  ##c(1, 3, 6, 12))
errorFun(iLAG)
...
u <- na.trim(cbind(data$rwPred3_CHUS, data$varPred3_CHUS, data$strPred3_CHUS,
                   data$GTSPred3_CHUS))
...
for (j in 1:4)
{
  #perf[,j]<- abs(data$EX_CHUS-u[,j]) #MAE 
